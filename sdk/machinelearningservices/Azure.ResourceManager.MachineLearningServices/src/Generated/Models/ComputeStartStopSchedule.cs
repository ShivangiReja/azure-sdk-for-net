// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

namespace Azure.ResourceManager.MachineLearningServices.Models
{
    /// <summary> Compute start stop schedule properties. </summary>
    public partial class ComputeStartStopSchedule
    {
        /// <summary> Initializes a new instance of ComputeStartStopSchedule. </summary>
        public ComputeStartStopSchedule()
        {
        }

        /// <summary> Initializes a new instance of ComputeStartStopSchedule. </summary>
        /// <param name="id"> Schedule id. </param>
        /// <param name="provisioningStatus"> The current deployment state of schedule. </param>
        /// <param name="status"> The schedule status. </param>
        /// <param name="triggerType"> The schedule trigger type. </param>
        /// <param name="action"> The compute power action. </param>
        /// <param name="recurrence"> The workflow trigger recurrence for ComputeStartStop schedule type. </param>
        /// <param name="cron"> The workflow trigger cron for ComputeStartStop schedule type. </param>
        internal ComputeStartStopSchedule(string id, ProvisioningStatus? provisioningStatus, ScheduleStatus? status, TriggerType? triggerType, ComputePowerAction? action, Recurrence recurrence, Cron cron)
        {
            Id = id;
            ProvisioningStatus = provisioningStatus;
            Status = status;
            TriggerType = triggerType;
            Action = action;
            Recurrence = recurrence;
            Cron = cron;
        }

        /// <summary> Schedule id. </summary>
        public string Id { get; }
        /// <summary> The current deployment state of schedule. </summary>
        public ProvisioningStatus? ProvisioningStatus { get; }
        /// <summary> The schedule status. </summary>
        public ScheduleStatus? Status { get; set; }
        /// <summary> The schedule trigger type. </summary>
        public TriggerType? TriggerType { get; set; }
        /// <summary> The compute power action. </summary>
        public ComputePowerAction? Action { get; set; }
        /// <summary> The workflow trigger recurrence for ComputeStartStop schedule type. </summary>
        public Recurrence Recurrence { get; set; }
        /// <summary> The workflow trigger cron for ComputeStartStop schedule type. </summary>
        public Cron Cron { get; set; }
    }
}
